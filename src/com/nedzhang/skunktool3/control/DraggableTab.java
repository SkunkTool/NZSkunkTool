 ...
 *         http://berry120.blogspot.com/2014/01/draggable-and-detachable
 *         -tabs-in-javafx.html#comment-form
 */
public class DraggableTab extends Tab {
	...
	public DraggableTab(final String text) {
		...
		nameLabel.setOnMouseDragged(new EventHandler<MouseEvent>() {
			...
			public void handle(final MouseEvent t) {
				...
				dragStage.show();
				final Point2D screenPoint = new Point2D(t.getScreenX(), t
						.getScreenY());
				tabPanes.add(getTabPane());
				final InsertData data = getInsertData(screenPoint);
				if (data == null || data.getInsertPane().getTabs().isEmpty()) {
					...
					final Rectangle2D rect = getAbsoluteRect(data
							.getInsertPane().getTabs().get(index));
					if (end) {
		...
		nameLabel.setOnMouseReleased(new EventHandler<MouseEvent>() {
			...
			public void handle(final MouseEvent t) {
				...
				if (!t.isStillSincePress()) {
					final Point2D screenPoint = new Point2D(t.getScreenX(), t
							.getScreenY());
					final TabPane oldTabPane = getTabPane();
					final int oldIndex = oldTabPane.getTabs().indexOf(
							DraggableTab.this);
					tabPanes.add(oldTabPane);
					final InsertData insertData = getInsertData(screenPoint);
					if (insertData != null) {
						int addIndex = insertData.getIndex();
						if (oldTabPane == insertData.getInsertPane()
								&& oldTabPane.getTabs().size() == 1) {
							return;
						}
						oldTabPane.getTabs().remove(DraggableTab.this);
						if (oldIndex < addIndex
								&& oldTabPane == insertData.getInsertPane()) {
							addIndex--;
						}
						if (addIndex > insertData.getInsertPane().getTabs()
								.size()) {
							addIndex = insertData.getInsertPane().getTabs()
									.size();
						}
						insertData.getInsertPane().getTabs()
								.add(addIndex, DraggableTab.this);
						insertData.getInsertPane().selectionModelProperty()
								.get().select(addIndex);
						return;
					}
					if (!detachable) {
	...
	private InsertData getInsertData(final Point2D screenPoint) {
		for (final TabPane tabPane : tabPanes) {
			final Rectangle2D tabAbsolute = getAbsoluteRect(tabPane);
			if (tabAbsolute.contains(screenPoint)) {
				int tabInsertIndex = 0;
				if (!tabPane.getTabs().isEmpty()) {
					final Rectangle2D firstTabRect = getAbsoluteRect(tabPane
							.getTabs().get(0));
					if (firstTabRect.getMaxY() + 60 < screenPoint.getY()
							|| firstTabRect.getMinY() > screenPoint.getY()) {
						return null;
					}
					final Rectangle2D lastTabRect = getAbsoluteRect(tabPane
							.getTabs().get(tabPane.getTabs().size() - 1));
					if (screenPoint.getX() < (firstTabRect.getMinX() + firstTabRect
							.getWidth() / 2)) {
						tabInsertIndex = 0;
					} else if (screenPoint.getX() > (lastTabRect.getMaxX() - lastTabRect
							.getWidth() / 2)) {
						...
						for (int i = 0; i < tabPane.getTabs().size() - 1; i++) {
							...
							if (leftTab instanceof DraggableTab
									&& rightTab instanceof DraggableTab) {
								final Rectangle2D leftTabRect = getAbsoluteRect(leftTab);
								final Rectangle2D rightTabRect = getAbsoluteRect(rightTab);
	...
	private Rectangle2D getAbsoluteRect(final Control node) {
		return new Rectangle2D(node.localToScene(
				node.getLayoutBounds().getMinX(),
				node.getLayoutBounds().getMinY()).getX()
				+ node.getScene().getWindow().getX(), node.localToScene(
				node.getLayoutBounds().getMinX(),
				node.getLayoutBounds().getMinY()).getY()
				+ node.getScene().getWindow().getY(), node.getWidth(),
				node.getHeight());
	}
	...
	private boolean betweenX(final Rectangle2D r1, final Rectangle2D r2,
			final double xPoint) {
