...
public class AutoCompleteComboBoxListener<T> implements EventHandler<KeyEvent> {
	...
	public void handle(final KeyEvent event) {
		...
		// length is the same.
		// this occurs if user types fast, the length of textfield will record
		// after the user
		...
		final IndexRange ir = comboBox.getEditor().getSelection();
		sb.delete(0, sb.length());
	 ...
	 */
	private void selectClosestResultBasedOnTextFieldValue(final boolean affect,
			final boolean inFocus) {
		final ObservableList<T> items = AutoCompleteComboBoxListener.this.comboBox
				.getItems();
		boolean found = false;
		for (int i = 0; i < items.size(); i++) {
			if (AutoCompleteComboBoxListener.this.comboBox.getEditor()
				...
				try {
					...
					final ListView<T> lv = ((ComboBoxListViewSkin<T>) AutoCompleteComboBoxListener.this.comboBox
							.getSkin()).getListView();
		...
		if (!inFocus && comboBox.getEditor().getText() != null
				&& comboBox.getEditor().getText().trim().length() > 0) {
			// press enter key programmatically to have this entry added
			@SuppressWarnings("deprecation")
			final KeyEvent ke = KeyEvent.impl_keyEvent(comboBox,
					KeyCode.ENTER.toString(), KeyCode.ENTER.getName(),
					KeyCode.ENTER.impl_getCode(), false, false, false, false,
