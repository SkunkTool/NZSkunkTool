...
public class SkunkSetting {
	...
	private SkunkSetting() {
		...
		if (VERBOSE_MODE) {
			System.out
					.println("STERLING_INTEROP_URL : " + STERLING_INTEROP_URL);
			System.out.println("STERLING_INTEROP_PROG_ID : "
					+ STERLING_INTEROP_PROG_ID);
			...
			System.out.println("STERLING_API_JAVADOC_URL : "
					+ STERLING_API_JAVADOC_URL);
			System.out.println("HARNESS_TO_LOAD : " + HARNESS_TO_LOAD);
		}

	}

	private String readAndTrimStringPorperty(final String propertyName) {
		final String propertyValue = ApplicationProperty.get(propertyName);

		return propertyValue == null || propertyValue.length() == 0 ? propertyValue
				: propertyValue.trim();
	}

	private boolean readBooleanProperty(final String propertyName) {
		final String propertyStringValue = readAndTrimStringPorperty(propertyName);
