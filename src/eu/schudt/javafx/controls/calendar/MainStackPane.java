...
final class MainStackPane extends StackPane {
	...
	public MainStackPane(final CalendarView calendarView) {
		...
		monthView = new AnimatedStackPane(new MonthView(calendarView),
				new MonthView(calendarView));
		yearView = new AnimatedStackPane(new YearView(calendarView),
				new YearView(calendarView));
		decadesView = new AnimatedStackPane(new DecadesView(calendarView),
				new DecadesView(calendarView));
		...
		calendarView.currentlyViewing.addListener(new ChangeListener<Number>() {
			...
			public void changed(
				...
				case Calendar.MONTH:
					...
					case Calendar.YEAR:
						calendarView.title.bind(yearView.actualPane
								.titleProperty());
				...
				case Calendar.YEAR:
					...
					case Calendar.MONTH:
						...
						calendarView.title.bind(monthView.actualPane
								.titleProperty());
						break;
					// Switch from decades to month.
					case Calendar.ERA:
						showOrHide(decadesView, true);
						calendarView.title.bind(decadesView.actualPane
								.titleProperty());
						break;
					}
					break;
	 ...
	 */
	private void showOrHide(final AnimatedStackPane stackPane,
			final boolean show) {
		stackPane.setVisible(true);

		calendarView.ongoingTransitions.set(calendarView.ongoingTransitions
				.get() + 1);
		final TranslateTransition translateTransition = new TranslateTransition(
				Duration.seconds(0.5), stackPane);
		final FadeTransition fadeTransition = new FadeTransition(
				Duration.seconds(0.5), stackPane);

		setClip(new Rectangle(stackPane.getBoundsInLocal().getWidth(),
		...
		parallelTransition.setOnFinished(new EventHandler<ActionEvent>() {
			...
			public void handle(final ActionEvent actionEvent) {
				...
				calendarView.ongoingTransitions
						.set(calendarView.ongoingTransitions.get() - 1);
