...
final class AnimatedStackPane extends StackPane {
	...
	public AnimatedStackPane(final DatePane actualPane,
			final DatePane animatePane) {
		...
		// Listen to changes of the calendar date, if it changes, check if the
		// new date has another month and move the panes accordingly.
		actualPane.calendarView.calendarDate
				.addListener(new ChangeListener<Date>() {
					@Override
					public void changed(
							final ObservableValue<? extends Date> observableValue,
							final Date oldDate, final Date newDate) {

						final Calendar calendar = actualPane.calendarView
								.getCalendar();
						...
						if (getWidth() > 0
								&& actualPane.calendarView.ongoingTransitions
										.get() == 0) {
							if (newYear > oldYear || newYear == oldYear
									&& newMonth > oldMonth) {
								slideLeftRight(-1, oldDate);
							} else if (newYear < oldYear || newYear == oldYear
									&& newMonth < oldMonth) {
								slideLeftRight(1, oldDate);
							}
						}
	 ...
	 * 
	 * @param direction
	 *            The direction, either 1 (moves to right) or -1 (moves to
	 *            left).
	 * @param oldDate
	...
	private void slideLeftRight(final int direction, final Date oldDate) {
		...
		final TranslateTransition transition1 = new TranslateTransition(
				Duration.seconds(SLIDE_ANIMATION_DURATION), animatePane);
		final TranslateTransition transition2 = new TranslateTransition(
				Duration.seconds(SLIDE_ANIMATION_DURATION), actualPane);
		...
		setClip(new Rectangle(animatePane.getBoundsInLocal().getWidth(),
				animatePane.getBoundsInLocal().getHeight()));

		// Move the old pane away from 0. (I added 1px, so that both panes
		// overlap, which makes it look a little smoother).
		transition1.setFromX(-direction * 1);
		// and either to right or to left.
		transition1.setToX(getLayoutBounds().getWidth() * direction
				+ -direction * 1);
		...
		slideTransition.setOnFinished(new EventHandler<ActionEvent>() {
			...
			public void handle(final ActionEvent actionEvent) {
				// When we are finished, set the animate pane to invisible and
				// remove the clip.
				animatePane.setVisible(false);
