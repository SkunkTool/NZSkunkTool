...
public class DatePicker extends HBox {
	...
	private void updateTextField() {
		// Mark the we update the text field (and not the user), so that it can
		// be ignored, by textField.textProperty()
		textSetProgrammatically = true;
		if (selectedDateProperty().get() != null) {
			final String date = getActualDateFormat().format(
	...
	private void showPopup() {
		...
		final Bounds calendarBounds = calendarView.getBoundsInLocal();
		final Bounds bounds = localToScene(getBoundsInLocal());

		final double posX = calendarBounds.getMinX() + bounds.getMinX()
				+ getScene().getX() + getScene().getWindow().getX();
		final double posY = calendarBounds.getMinY() + bounds.getHeight()
				+ bounds.getMinY() + getScene().getY()
