...
final class MonthView extends DatePane {
	...
	/**
	 * The number of days per week. I don't know if there is a culture with more
	 * or less than seven days per week, but theoretically {@link Calendar}
	 * allows it. This variable will correspond to the number of columns.
	 */
	...
	public MonthView(final CalendarView calendarView) {
		...
		calendarView.getDisabledWeekdays().addListener(
		...
		// When the disabled dates change, update the days.
		calendarView.showWeeksProperty().addListener(
				new InvalidationListener() {
					@Override
					public void invalidated(final Observable observable) {
						getChildren().clear();
	...
	public void buildContent() {
		final Calendar calendar = calendarView.calendarProperty().get();
		...
		final int maxNumberOfDaysInMonth = calendar
				.getMaximum(Calendar.DAY_OF_MONTH);

		// assume the first row has only 1 day, then distribute the rest among
		// the remaining weeks and add the first week.
		final int numberOfRows = (int) Math.ceil((maxNumberOfDaysInMonth - 1)
				/ (double) numberOfDaysPerWeek) + 1;

		// remove all controls
		getChildren().clear();

		final int colOffset = calendarView.getShowWeeks() ? 1 : 0;
			...
			for (int colIndex = 0; colIndex < numberOfDaysPerWeek; colIndex++) {
				...
				button.setOnAction(new EventHandler<ActionEvent>() {
					...
					public void handle(final ActionEvent actionEvent) {
						...
						calendarView.selectedDate.set((Date) button
								.getUserData());
					}
				});
	...
	private void updateDays() {
		...
		final DateFormat dateFormat = DateFormat.getDateInstance(
				DateFormat.FULL, calendarView.localeProperty().get());
		...
		for (int i = numberOfDaysPerWeek
			...
			if (i % (numberOfDaysPerWeek + 1) == 0
				...
				final Label label = (Label) getChildren().get(i);
				label.setText(Integer.toString(calendar
						.get(Calendar.WEEK_OF_YEAR)));
			} else {
				final Button control = (Button) getChildren().get(i);
				...
				for (final Date disabledDate : calendarView.getDisabledDates()) {
					...
					if (calendar.get(Calendar.YEAR) == clone.get(Calendar.YEAR)
							&& calendar.get(Calendar.MONTH) == clone
									.get(Calendar.MONTH)
							&& calendar.get(Calendar.DAY_OF_MONTH) == clone
									.get(Calendar.DAY_OF_MONTH)) {
						disabled = true;
					}
				}
				...
				if (calendar.get(Calendar.YEAR) == todayYear
						...
						&& calendar.get(Calendar.DATE) == todayDay) {
	...
	private void updateWeekNames() {
		final DateFormatSymbols dateFormatSymbols = new DateFormatSymbols(
				calendarView.localeProperty().get());
		final String[] weekDays = dateFormatSymbols.getShortWeekdays();

		// Start with 1 instead of 0, since the first element in the array is
		// empty.
		for (int i = 1; i < weekDays.length; i++) {
			// Get the first character only.

			// FIX submitted by Masayuki Ueki for Japanese language as they only
			// have one character
			String shortWeekDay = "";
			...
			final Label label = (Label) getChildren().get(
					j + (calendarView.getShowWeeks() ? 1 : 0));

			label.setText(shortWeekDay);
		}
		title.set(getDateFormat("MMMM yyyy").format(
				calendarView.getCalendar().getTime()));
	}
}
