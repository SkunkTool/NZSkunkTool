...
final class YearView extends DatePane {
	...
	protected void buildContent() {
		...
		// Get the number of months. I read, there are some lunar calendars,
		// with more than 12 months.
		final int numberOfMonths = calendarView.getCalendar().getMaximum(
				Calendar.MONTH) + 1;
			...
			button.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(final ActionEvent actionEvent) {
					if (calendarView.currentlyViewing.get() == Calendar.YEAR) {
						...
						calendarView.calendarDate.set(calendarView
								.getCalendar().getTime());
					}
				}
			});
	...
	protected void updateContent() {
		final DateFormatSymbols symbols = new DateFormatSymbols(calendarView
				.localeProperty().get());
		final String[] monthNames = symbols.getShortMonths();
		for (int i = 1; i < monthNames.length; i++) {
			final Button button = (Button) getChildren().get(i - 1);
			button.setText(monthNames[i - 1]);
		}
		title.set(getDateFormat("yyyy").format(
